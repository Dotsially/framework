...
Ray RaycastMouseWorldSpace(Camera* camera, Window* window, InputState* state) {
    ...
    float ndcX = (2.0f * state->mousePosition.x) / window->width - 1.0f;
    float ndcY = 1.0f - (2.0f * state->mousePosition.y) / window->height; // Y is inverted because window coordinates start from top-left
