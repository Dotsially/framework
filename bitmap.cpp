...
public:
    ...
    void Load(TexturePacker* packer, std::string fontFileName) {
        ...
        std::string atlasTextureName = packer->AddTexture(textureFile);

        // Get the UV data for the font image in the atlas
        TextureData texData = packer->GetTexture(atlasTextureName);

        int atlasSize = packer->m_atlasSize;
        ...
        for (int ascii = asciiStart; ascii <= asciiEnd; ++ascii) {
            ...
            m_glyphs[static_cast<char>(ascii)] = glyph;
            ++glyphIndex;
        }
    }
};
